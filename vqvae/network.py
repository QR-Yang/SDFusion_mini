...
class VQVAE(nn.Module):
    ...
    def decode_enc_idices(self, enc_indices, z_spatial_dim=8):
        ...
        # for transformer
        enc_indices = enc_indices.permute(1, 0).contiguous().view(-1)
        z_q = self.quantize.embedding(enc_indices) # (bs t) zd
        batch_size = z_q.shape[0] // (z_spatial_dim * z_spatial_dim * z_spatial_dim)
        z_q = z_q.view(batch_size, z_spatial_dim, z_spatial_dim, z_spatial_dim, -1)
